...
app.get('/api/videos', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT v.id, v.title, v.description, v.thumbnail, v.created_at, v.schedule_at, v.likes, v.views,
...
app.get("/api/videos/:id", blockScheduledVideoAccess, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `
      SELECT v.id, v.title, v.description, v.video_path, v.thumbnail, v.views, v.likes, v.created_at, v.schedule_at,
             u.id as user_id, u.username, u.first_name, u.last_name
      FROM videos v
      JOIN users u ON v.user_id = u.id
      WHERE v.id = $1
